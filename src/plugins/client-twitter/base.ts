...
export class ClientBase extends EventEmitter {
  ...
  async getTweet(tweetId: string): Promise<Tweet> {
    ...
    const tweet = await this.handleError32(
      this.requestQueue.add(() => this.twitterClient.getTweet(tweetId))
    );
  ...
  private parseTweet(raw: any, depth = 0, maxDepth = 3): Tweet {
    ...
    const t: Tweet = {
      ...
      //@ts-expect-error - missing in agent-twitter-client
  ...
  async handleError32<T>(func: Promise<T>): Promise<T> {
    try {
      return await func;
    } catch (error: any) {
      if (error?.data?.errors) {
        const firstError = error?.data?.errors[0];
        if (firstError && firstError.code === 32) {
          await this.init();
        }
      }
  ...
  async fetchOwnPosts(count: number): Promise<Tweet[]> {
    ...
    const homeTimeline = await this.handleError32<any>(
      this.twitterClient.getUserTweets(this.profile.id, count)
    );
  ...
  async fetchSearchTweets(
    ...
    try {
      ...
      try {
        const result = await this.handleError32(
  ...
  async fetchProfile(username: string): Promise<TwitterProfile> {
    try {
      const profile = await this.handleError32(
