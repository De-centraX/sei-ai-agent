...
# Task: Generate a post in the voice and style and perspective of {{agentName}} @{{twitterUserName}} without duplicating previous post.
Write a post that is {{adjective}} about {{topic}} (without mentioning {{topic}} directly), from the perspective of {{agentName}}. Do not add commentary or acknowledge this request, just write the post.
Your response should be 1, 2, or 3 sentences (choose the length at random).
Your response should not include the agent's name.
Your response should not be a duplication of a previous tweet/post.
Your response should never contain buzz, buzzing, buzzin' and its derivatives.
Your response should not contain hashtags.
...
- Skip ALL content that is:
  - It is a joke
  - In ecosystem different than character's
...
export class TwitterPostClient {
  ...
  async generateNewTweet() {
    ...
    try {
      ...
      const supportedActions = ['SCRAPE_KEYWORD', 'NOTABLE_ACCOUNTS_TWEET'];
      const supportedActionsOptions = {
        SCRAPE_KEYWORD: {
          keyword:
            this.runtime.character.settings?.secrets?.TWITTER_SEARCH_KEYWORDS,
          client: this.client,
        },
        NOTABLE_ACCOUNTS_TWEET: {
          client: this.client,
        },
      };
      const randomActionName =
        supportedActions[Math.floor(Math.random() * supportedActions.length)];

      const action = this.runtime.actions.find(
        (a) => a.name === randomActionName
      );

      if (!action) {
        elizaLogger.error('No action found for:', randomActionName);
        return;
      }
      ...
      const state = await this.runtime.composeState(
        ...
        { twitterUserName: this.client.profile.username, maxTweetLength }
      );
      ...
      const callback: HandlerCallback = async (response: Content) => {
        ...
        const rawTweetContent = cleanJsonResponse(response.text);
        ...
        // Fix missing spaces after periods, question marks, and exclamation points
        const fixSentenceSpacing = (str: string) =>
          str.replace(/([.!?])([A-Z])/g, '$1 $2');
        const fixDoubleSpaces = (str: string) => str.replace(/  /g, ' ');

        // Final cleaning
        tweetTextForPosting = fixDoubleSpaces(
          removeQuotes(fixNewLines(fixSentenceSpacing(tweetTextForPosting)))
        ).trim();
      ...
      action.handler(
        this.runtime,
        { action: 'TWEET_POST' } as unknown as Memory,
        state,
        supportedActionsOptions[randomActionName],
        callback
      );
    } catch (error) {
    ...
    try {
      ...
      for (const tweet of timelines) {
        ...
        try {
          ...
          if (memory) {
            elizaLogger.info(`Already processed tweet ID: ${tweet.id}`);
            continue;
          }
      ...
      try {
        ...
        if (actionResponse.quote) {
          try {
            ...
            elizaLogger.info('Generated quote tweet content:', quoteContent);
            ...
            if (this.isDryRun) {
              ...
              if (body?.data?.create_tweet?.tweet_results?.result) {
                elizaLogger.info('Successfully posted quote tweet');
